...
from data import TrainDataset,TestDataset,process_data
...
if __name__ == '__main__':
    ...
    # data = process_data(config.train_path,config.classes2idx,label=True)
    # train_df,val_df = train_eval_data(data_df=data,ratio_eval=0.2,shuffle=True,random_state=1) #random_state提供随机种子，保证每次的结果相同
    #
    ...
    # data = process_data(config.test_path, config.classes2idx, label=False)
    # test_dataset = TrainDataset(data,label=False)
